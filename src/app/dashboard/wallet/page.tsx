    ...
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
            ...
            <p className="text-muted-foreground font-medium mt-1">Manage your funds and deposit proof securely.</p>
        </div>
        <button 
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1">
          <motion.div 
            ...
            className="bg-card text-card-foreground p-8 rounded-3xl border border-border shadow-md group hover:border-primary/50 transition-all flex items-center justify-between"
          >
            <div className="flex items-center gap-6">
              <div className="h-16 w-16 rounded-2xl bg-primary/10 flex items-center justify-center border border-primary/20 group-hover:scale-110 transition-transform">
                <Wallet className="h-8 w-8 text-primary" />
              </div>
              <div>
                <p className="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground mb-1">Available Balance</p>
                <h3 className="text-4xl font-black font-mono tracking-tighter">{formatPrice(data?.profile?.balance || 0)}</h3>
              </div>
            </div>
          </motion.div>
        </div>
      ...
      {showDeposit && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
          <motion.div 
            ...
            className="w-full max-w-4xl bg-card text-card-foreground rounded-[3rem] border border-border overflow-hidden shadow-2xl my-8"
          >
            <div className="p-8">
              <div className="flex justify-between items-start mb-8">
                <div>
                  <h3 className="text-3xl font-black font-outfit tracking-tighter text-foreground mb-2">Deposit Funds</h3>
                  <div className="flex items-center gap-2 text-muted-foreground font-bold uppercase tracking-[0.2em] text-[10px]">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-5 gap-12 items-stretch relative z-10">
                ...
                <div className="lg:col-span-2 flex flex-col gap-6">
                  <div className="p-8 rounded-[2.5rem] bg-muted/30 border border-border shadow-sm space-y-8 flex-1">
                    <div className="space-y-2">
                        ...
                        <p className="text-[11px] text-muted-foreground font-medium">Use the details below for your transfer.</p>
                    </div>
                    ...
                    <div className="space-y-5">
                      {[
                        ...
                        <div key={item.label} className="group/item relative">
                          <p className="text-[9px] uppercase font-black text-muted-foreground mb-2 tracking-[0.3em] px-1">{item.label}</p>
                          <div className="flex justify-between items-center p-4 rounded-2xl bg-card border border-border group-hover/item:border-primary/40 transition-all">
                            <p className={cn("text-sm font-bold truncate", item.mono && "font-mono text-primary text-lg tracking-wider")}>{item.value}</p>
                            <button onClick={() => copyToClipboard(item.value)} className="p-2.5 bg-muted hover:bg-primary/20 rounded-xl text-muted-foreground hover:text-primary transition-all active:scale-90">
                  ...
                  <div className="p-6 rounded-3xl bg-amber-500/5 border border-amber-500/10 flex gap-4 backdrop-blur-sm">
                    ...
                    <div>
                      <p className="text-[10px] font-black text-amber-500 uppercase tracking-widest mb-1">Admin Verification</p>
                ...
                <div className="lg:col-span-3">
                  <div className="h-full flex flex-col p-10 md:p-12 rounded-[2.5rem] bg-muted/10 border border-border shadow-sm relative">
                    <div className="space-y-10">
                      <div className="space-y-8">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                          <div className="space-y-3">
                            <label className="text-[10px] uppercase font-black text-muted-foreground px-2 tracking-[0.2em]">Deposit Amount</label>
                            <div className="relative">
                                <span className="absolute left-5 top-1/2 -translate-y-1/2 text-primary font-mono font-black text-xl">₦</span>
                                <input 
                                    type="number" 
                                    placeholder="5,000"
                                    id="depositAmount"
                                    className="w-full bg-card border border-border rounded-2xl pl-12 pr-6 py-5 focus:outline-none focus:ring-2 focus:ring-primary/40 transition-all font-mono text-primary text-2xl placeholder:text-primary/20"
                                />
                            </div>
                          </div>
                          <div className="space-y-3">
                            <label className="text-[10px] uppercase font-black text-muted-foreground px-2 tracking-[0.2em]">Transfer Ref</label>
                            <input 
                              type="text" 
                              placeholder="Your Name / Ref"
                              id="paymentRef"
                              className="w-full bg-card border border-border rounded-2xl px-6 py-5 focus:outline-none focus:ring-2 focus:ring-primary/40 transition-all font-bold placeholder:text-muted-foreground/30 text-foreground"
                            />
                          </div>
                        </div>

                        <div className="space-y-4">
                          <label className="text-[10px] uppercase font-black text-muted-foreground px-2 tracking-[0.2em]">Upload Receipt</label>
                          <label htmlFor="screenshot" className="group block cursor-pointer">
                            <div className="flex flex-col items-center justify-center p-8 border-2 border-dashed border-border rounded-3xl group-hover:border-primary/40 group-hover:bg-primary/5 transition-all text-center">
                                <Plus className="h-8 w-8 text-muted-foreground/20 mb-3 group-hover:text-primary transition-colors" />
                                <p className="text-xs font-bold text-muted-foreground group-hover:text-foreground transition-colors">Select screenshot proof from your files</p>
                                <input 
                                    type="file" 
                                    id="screenshot"
                                    accept="image/*"
